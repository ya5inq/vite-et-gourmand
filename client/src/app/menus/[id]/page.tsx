...
import type { PublicMenuGetOne200 } from '@vite-et-gourmand/sdk';
...
const DISH_CATEGORY_ORDER = ['entree', 'plat', 'dessert'];
...
async function fetchMenu(id: string): Promise<PublicMenuGetOne200 | null> {
  const api = getServerPublicApi();
  try {
    const { data } = await api.publicMenuGetOne(id);
    return data;
  } catch (error) {
...
export default async function MenuDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const regimes = menu.dietaryRegimes ?? [];

  const dishesByCategory = DISH_CATEGORY_ORDER.reduce<Record<string, typeof dishes>>(
    (acc, cat) => {
      const filtered = dishes.filter((d) => d.category === cat);
  ...
  const imageUrl = isValidImageUrl(menu.imageUrl ?? null)
    ? menu.imageUrl!
    : getDefaultImage(menu.id);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden">
        <div className="relative h-64 sm:h-80">
          <Image src={imageUrl} alt={menu.name} fill priority className="object-cover" />
          {menu.theme && (
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-8">
            <div className="bg-secondary rounded-lg p-4">
              ...
              <p className="font-semibold text-foreground">
                Min. {menu.minPersons}
                {menu.maxPersons && ` - Max. ${menu.maxPersons}`}
              </p>
            </div>
            {menu.conditions && (
              <div className="bg-secondary rounded-lg p-4">
              ...
              {Object.entries(dishesByCategory).map(([category, categoryDishes]) => (
                <div key={category}>
                  <h3 className="text-lg font-medium text-primary mb-3 border-b border-border pb-2">
                    {DISH_CATEGORY_LABELS[category] ?? category}
                  </h3>
                        ...
                        <div key={dish.id} className="pl-4">
                          ...
                          {allergens.length > 0 && (
                            <div className="flex gap-1.5 mt-1">
                              {allergens.map((a) => {
                                const icon =
                                  a.icon || ALLERGEN_ICONS[a.name.toLowerCase()] || null;
                                return (
                                  <span
                                    key={a.id}
