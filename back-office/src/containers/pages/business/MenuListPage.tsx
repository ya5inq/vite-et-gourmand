...
import { useMenus, useCreateMenu, useUpdateMenu, useDeleteMenu, type MenuRow } from '@/api/hooks/useMenus';
...
export const MenuListPage = () => {
  ...
  const onSubmit = (data: MenuForm) => {
    if (editingMenu) {
      updateMenu.mutate({ id: editingMenu.id, ...data }, { onSuccess: () => setIsModalOpen(false) });
    } else {
      createMenu.mutate(data, { onSuccess: () => setIsModalOpen(false) });
    }
  };
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-lg bg-card p-6 shadow-lg">
            <h2 className="mb-4 text-lg font-bold">{editingMenu ? 'Modifier le menu' : 'Nouveau menu'}</h2>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  {createMenu.isPending || updateMenu.isPending ? 'Enregistrement...' : 'Enregistrer'}
                </button>
              </div>
            </form>
          </div>
