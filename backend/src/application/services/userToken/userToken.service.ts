...
import { GenerateTokenOptions, UserTokenServiceInterface, VerifyTokenOptions } from './userToken.service.interface';

@injectable()
export class UserTokenService implements UserTokenServiceInterface {
  constructor() {}

  generateToken({ userId, expiresIn, canBeRefreshed, tokenType }: GenerateTokenOptions): UserTokenInterface {
    const tokenValue = v4();
    return {
      id: v4(),
      userId,
  ...
  verifyToken({
    ...
    if (token.expirationDate < new Date() && !ignoreExpiration) {
      throw new AppError({
        code: AppErrorCodes.UNAUTHORIZED_TOKEN_EXPIRED,
    ...
    if (token.tokenType !== tokenType) {
      throw new AppError({
        code: AppErrorCodes.FORBIDDEN_TOKEN_TYPE_MISMATCH,
    ...
    if (userId && token.userId !== userId) {
      throw new AppError({
        code: AppErrorCodes.FORBIDDEN_TOKEN_USER_MISMATCH,
