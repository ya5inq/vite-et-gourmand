...
export class StatelessTokenService implements StatelessTokenServiceInterface {
  async generateToken({ payload, secret, expiresIn }: GenerateStatelessTokenOptions): Promise<string> {
  ...
  async verifyToken({ token, secret, ignoreExpiration = false }: VerifyStatelessTokenOptions): Promise<unknown> {
