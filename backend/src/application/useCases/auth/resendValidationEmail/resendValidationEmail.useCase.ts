...
export class ResendValidationEmailUseCase implements ResendValidationEmailUseCaseInterface {
  ...
  async executeResendValidationEmail(params: { email?: string; token?: string }): Promise<void> {
    ...
    if (user.emailVerified) {
      throw new AppError({
        ...
        code: AppErrorCodes.CONFLICT_ACCOUNT_ALREADY_VALIDATED,
      });
    }
    ...
    this.loggerService.debug(`Resend validation email successful: New token generated for user ${user.id}`);
  }
  ...
  private async generateAndSaveNewToken(user: UserInterface, userToken: UserTokenInterface | null): Promise<string> {
    const newTokenValue = v4();
    if (userToken) {
      await this.userTokenRepository.update({
        oldTokenValue: userToken.value,
        newTokenValue,
        expirationDate: new Date(Date.now() + 1000 * this.envConfig.accountTokenExpiration),
      });
    } else {
