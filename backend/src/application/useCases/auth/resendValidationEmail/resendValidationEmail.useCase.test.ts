...
describe('ResendValidationEmailUseCase', () => {
  ...
  describe('executeResendValidationEmail', () => {
    ...
    it('should throw an error if neither email nor token is provided', async () => {
      await expect(resendValidationEmailUseCase.executeResendValidationEmail({})).rejects.toThrow(AppError);
    });
    ...
    it('should throw an error if user is not found with provided email', async () => {
      ...
      await expect(resendValidationEmailUseCase.executeResendValidationEmail({ email })).rejects.toThrow(AppError);
    });
    ...
    it('should throw an error if email is already verified', async () => {
      ...
      await expect(resendValidationEmailUseCase.executeResendValidationEmail({ email: user.email })).rejects.toThrow(
        AppError,
      );
    });
  });
