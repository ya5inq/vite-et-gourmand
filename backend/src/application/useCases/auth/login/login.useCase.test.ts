...
describe('LoginUseCase', () => {
  ...
  it('should return access and refresh token', async () => {
    ...
    const user = userFactory({ password: clearPassword, isActive: true });
    const refreshTokenMock = userTokenFactory({ tokenType: UserTokenTypeEnum.refreshToken });

    userRepository.findByEmail.mockResolvedValue(user);
    passwordService.comparePassword.mockResolvedValue(true);
    authService.generateAccessToken.mockResolvedValue('access-token');
    authService.generateRefreshToken.mockReturnValue(refreshTokenMock);

    const { accessToken, refreshToken } = await loginUseCase.executeLogin(user.email, clearPassword);
    ...
    expect(authService.generateAccessToken).toHaveBeenCalledWith(user.id, user);
    expect(authService.generateRefreshToken).toHaveBeenCalledWith(user.id);
    expect(userTokenRepository.create).toHaveBeenCalledWith(refreshTokenMock);
  });

  it('should throw an error if user not found', async () => {
    userRepository.findByEmail.mockResolvedValue(null);

    await expect(loginUseCase.executeLogin(faker.internet.email(), faker.internet.password())).rejects.toThrow(
      AppError,
    );
  });

  it('should throw an error if account is disabled', async () => {
    const user = userFactory({ isActive: false });
    userRepository.findByEmail.mockResolvedValue(user);

    await expect(loginUseCase.executeLogin(user.email, faker.internet.password())).rejects.toThrow(AppError);
    expect(passwordService.comparePassword).not.toHaveBeenCalled();
  });

  it('should throw an error if password is invalid', async () => {
    const user = userFactory({ isActive: true });
    userRepository.findByEmail.mockResolvedValue(user);
