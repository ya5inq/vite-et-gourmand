...
describe('ResetPasswordUseCase', () => {
  ...
  describe('executeResetPasswordRequest', () => {
    it('should send an email with a reset password link', async () => {
      ...
      expect(userTokenService.generateToken).toHaveBeenCalledWith({
        ...
        expiresIn: envConfig.resetTokenExpiration,
      });
    ...
    it('should not throw and not send email if user not found (no enumeration)', async () => {
      userRepository.findByEmail.mockResolvedValue(null);

      await expect(resetPasswordUseCase.executeResetPasswordRequest(faker.internet.email())).resolves.toBeUndefined();
      expect(mailSender.sendResetPasswordEmail).not.toHaveBeenCalled();
    });
  });
  ...
  describe('executeResetPassword', () => {
    ...
    it('should throw an error if password is invalid', async () => {
      ...
      await expect(resetPasswordUseCase.executeResetPassword(token.value, faker.internet.password())).rejects.toThrow(
        AppError,
      );
    });
