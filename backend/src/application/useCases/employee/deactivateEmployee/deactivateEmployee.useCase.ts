...
import {
  DeactivateEmployeeInputInterface,
  DeactivateEmployeeUseCaseInterface,
} from './deactivateEmployee.useCase.interface';

@injectable()
export class DeactivateEmployeeUseCase implements DeactivateEmployeeUseCaseInterface {
  constructor(
  ...
  async executeDeactivateEmployee({
    employeeId,
    actorId,
    actorRole,
  }: DeactivateEmployeeInputInterface): Promise<UserInterface> {
    ...
    // Never allow disabling an admin account through this route.
    if (employee.role === RoleType.ADMIN) {
      throw new AppError({
        code: AppErrorCodes.BAD_REQUEST_CANNOT_DEACTIVATE_ADMIN,
    ...
    await this.userRepository.updateOne(employeeId, { isActive: false });
    ...
    this.logger.debug('Employee deactivated', { employeeId });
