...
import { UserTokenInterface, UserTokenType } from '@/domain/entities/userToken/userToken.entity.interface';
import {
...
export class UserTokenRepository implements UserTokenRepositoryInterface {
  ...
  update = async ({ oldTokenValue, newTokenValue, expirationDate }: UpdateTokenOptions): Promise<void> => {
    await this.repository.update({ value: oldTokenValue }, { value: newTokenValue, expirationDate });
  };
