...
import { adminOperatingHoursGetAllSchema } from './schema';

const adminOperatingHoursGetAllRoute = getHonoApp();

const route = createRoute({
  method: 'get',
  path: '/operating-hours',
  tags: ['admin', 'cms'],
  operationId: 'AdminOperatingHoursGetAll',
  summary: 'CMS - List operating hours (staff)',
  responses: {
    200: jsonSuccessGetResponse(adminOperatingHoursGetAllSchema.response),
    401: jsonErrorResponse(AppErrorCodes.UNAUTHORIZED),
    403: jsonErrorResponse(AppErrorCodes.FORBIDDEN),
  },
});

adminOperatingHoursGetAllRoute.openapi(route, async (c) => {
...
export { adminOperatingHoursGetAllRoute };
